...
from wml.iotoolkit.labelme_toolkit import LabelMeData,save_labelme_datav3,save_detdata
...
def cut_around_bboxes_and_save(dataset,data_dir,save_dir,cut_size,keep_ratio=1e-6):
    ...
    for idx,data in enumerate(dataset):
        raw_detdata = copy.deepcopy(data)
        ...
        for i in range(len(labels_names)):
            new_img,new_detdata = odp.cut_detdata(cut_bboxes[i],img,raw_detdata,keep_ratio=keep_ratio)
            ...
            save_detdata(ann_save_path,img_save_path,new_detdata)
    ...
    for idx,data in enumerate(dataset):
        ...
        while y0<img.shape[0]:
            ...
            while x0<img.shape[1]:
                cut_bbox = np.array((x0,y0,x0+cut_size[0],y0+cut_size[1]))
                new_img,new_detdata = odp.cut_detdata(cut_bbox,img,raw_detdata,keep_ratio=keep_ratio)

                if len(new_detdata.labels) == 0 and not save_empty:
                    x0 += sx 
                    continue
