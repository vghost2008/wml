...
class CocoToolsTest(tf.test.TestCase):
  ...
  def testExportGroundtruthToCOCO(self):
    ...
    groundtruth_boxes = [np.array([[100, 100, 200, 200]], np.float32),
  ...
  def testExportDetectionsToCOCO(self):
    image_ids = ['first', 'second']
    detections_boxes = [np.array([[100, 100, 200, 200]], np.float32),
                        np.array([[50, 50, 100, 100]], np.float32)]
    detections_scores = [np.array([.8], np.float32), np.array([.7], np.float32)]
  ...
  def testExportKeypointsToCOCO(self):
    ...
    detection_scores = [np.array([.8, 0.2], np.float32),
