...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'GT_IMG_LOC_FORMAT': '{gt_folder}/{seq}/img1',  # '{gt_folder}/{seq}/gt/gt.txt'
    ...
    def __init__(self, config=None):
        ...
        try:
            ...
            self.seq_list, self.seq_lengths = self._auto_make_seq_info()
        ...
        if self.config["SEQ_INFO"]:
            ...
            with open(seqmap_file) as fp:
                ...
                for i, row in enumerate(reader):
                    ...
                    ini_file = os.path.join(self.gt_fol, seq, 'seqinfo.ini')
                    if not os.path.isfile(ini_file):
                        print(f"ERROR: file {ini_file} not exist.")
                        raise TrackEvalException('ini file does not exist: ' + seq + '/' + os.path.basename(ini_file))
                    ini_data = configparser.ConfigParser()
                    ini_data.read(ini_file)
                    seq_lengths[seq] = int(ini_data['Sequence']['seqLength'])
    ...
    def _auto_make_seq_infov1(self):
        ...
        seq_lengths = {}
        for seq in _seqs:
            img_dir = self.config["GT_IMG_LOC_FORMAT"].format(gt_folder=self.gt_fol,seq=seq)
            if not osp.isdir(img_dir):
                print(f"ERROR:{img_dir} is not a dir.")
                continue
            imgs_nr = len(glob.glob(osp.join(img_dir,"*.jpg")))
            if imgs_nr ==0:
                print(f"ERROR: Find imgs in {img_dir} faild.")
                continue
            seq_list.append(seq)
            seq_lengths[seq] = imgs_nr
